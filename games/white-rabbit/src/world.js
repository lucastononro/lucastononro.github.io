...
import {installStaticBatching,disposeStaticBatching} from './static-batching.js';
...
export class World{
 ...
 async load(room,state){
  disposeStaticBatching(this);
  if(room!=='hall')state.size=has(state,'mushroom')?33:11;
  this.wasInsideLittleDoor=false;
  ...
  this.doorLeaf=null;this.hotspots.small_door?.traverse(o=>{if(o.userData.doorLeaf==='small_door')this.doorLeaf=o});if(this.doorLeaf)this.doorLeaf.rotation.y=has(state,'door_unlock')?-Math.PI*.46:0;
  ...
  await installAnimals(this);await installResidents(this);
  installStaticBatching(this);
 ...
 updateComposition(){const dialog=document.querySelector('#modal-backdrop');const panel=!dialog?.classList.contains('hidden')?document.querySelector('#modal'):document.querySelector('#manipulation');const width=panel?.getBoundingClientRect().width||0;const clear=dialog?.classList.contains('clue-view')||document.body.classList.contains('touch')&&document.body.classList.contains('dialog-open');if(this.examining&&!clear&&width<innerWidth*.8)this.camera.setViewOffset(innerWidth,innerHeight,(width+24)/2,0,innerWidth,innerHeight);else this.camera.clearViewOffset();}
 ...
 valid(x,z){const r=this.room;const xMax=r==='hall'?8.35:r==='tower'?9.2:13.2;const zMin=r==='hall'?-10.65:r==='tower'?-10:-14.1;const zMax=r==='hall'?11.2:r==='tower'?10:14;
  if(this.overworld){if(!insideValley(x,z,this.state))return false}else{const passage=r==='hall'&&has(this.state,'door_unlock')&&canPassLittleDoor(this.state.size)&&Math.abs(x-LITTLE_DOOR.x)<LITTLE_DOOR.width/2-.1&&z>=-13.5&&z<zMin;if(!passage&&(Math.abs(x)>xMax||z<zMin||z>zMax))return false;}
 ...
 enterLittleDoor(){if(!has(this.state,'door_unlock')||!canPassLittleDoor(this.state.size))return;this.navigation={path:[new THREE.Vector3(LITTLE_DOOR.x,this.height,-10.5),new THREE.Vector3(LITTLE_DOOR.x,this.height,-12.55)],index:0,center:new THREE.Vector3(LITTLE_DOOR.x,this.height,-14),done:()=>this.callbacks.onDoorPass?.()};}
 ...
 examine(id,custom){
  ...
  if(id==='letter'&&!custom)custom={position:[0,2.5,-3.8],target:[0,2.35,-11.48]};
  ...
  this.pause(true);this.examining=true;this.updateComposition();const box=this.boxes[id]?.clone();if(id==='caterpillar'&&this.smokeDigits)box.expandByObject(this.smokeDigits);const target=custom?.target?new THREE.Vector3(...custom.target):box.getCenter(new THREE.Vector3());const size=box?.getSize(new THREE.Vector3())||new THREE.Vector3(2,2,2);
 ...
 animate(){
  ...
  if(this.doorLeaf)this.doorLeaf.rotation.y=THREE.MathUtils.damp(this.doorLeaf.rotation.y,has(this.state,'door_unlock')?-Math.PI*.46:0,5,dt);
