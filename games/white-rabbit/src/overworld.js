...
export function addValley(scene){
 ...
 const ribbon=(points,width,mat='path')=>{const curve=new THREE.CatmullRomCurve3(points.map(p=>new THREE.Vector3(...p)));const pts=curve.getPoints(100);const vertices=[],indices=[];for(let i=0;i<pts.length;i++){const tang=curve.getTangent(i/100);const n=new THREE.Vector3(-tang.z,0,tang.x).normalize().multiplyScalar(width/2);for(const dir of [-1,1]){const p=pts[i].clone().addScaledVector(n,dir);vertices.push(p.x,p.y,p.z)}if(i<100){const a=i*2;indices.push(a,a+1,a+2,a+1,a+3,a+2)}}const geo=new THREE.BufferGeometry();geo.setAttribute('position',new THREE.Float32BufferAttribute(vertices,3));geo.setIndex(indices);geo.computeVertexNormals();const o=new THREE.Mesh(geo,mats[mat]);o.receiveShadow=true;group.add(o);return curve};
 ...
 ribbon([[0,.04,-26],[11,.04,-29],[23,.04,-25],[32,.04,-22],[48,.04,-22],[59,.04,-22]],3.8);
 ...
 for(const x of [58,86]){box([x,3,-29],[1.5,6,14],'stone');for(let z=-35;z<-22;z+=2)box([x,6.3,z],[1.7,.85,.95],'stone')}
