...
<article class="post">
  ...
  ${post.game ? `<section class="game-launch" aria-label="Play After the White Rabbit">
    <a class="game-poster" href="${u('/games/white-rabbit/')}" aria-label="Start playing After the White Rabbit"><img src="${u('/images/white-rabbit-game.png')}" alt="The modeled hall of little doors in After the White Rabbit" width="1280" height="720" fetchpriority="high"><span class="game-start">Start playing <span aria-hidden="true">↗</span></span></a>
    <div class="game-launch-caption"><strong>After the White Rabbit</strong><span>First-person puzzle adventure · 4 chapters · Desktop & phone</span><small>On phones: landscape, left thumb to walk, right thumb to look and interact. Saves in your browser.</small></div>
  </section>` : ''}
...
/**
 * The comment section. Only the empty shell is rendered here: theme.js injects
 * the giscus script once it knows which way the switch is pointing, so the
 * frame comes up in the right colours instead of flashing the wrong ones. That
 * also means nothing is requested from giscus.app until the script runs, which
 * keeps the promise the footer makes about the video embeds.
 */
function comments() {
  ...
  const config = {
    ...
    mapping: c.mapping,
    lang: c.lang,
    themeLight: abs('/giscus-light.css'),
    themeDark: abs('/giscus-dark.css'),
  }
  ...
  return `<section class="comments" id="comments">
    <p class="section-label"><span>Comments</span></p>
    <p class="comments-note">Sign in with GitHub and say something. Threads live
    in this repository's <a href="https://github.com/${escapeHtml(c.repo)}/discussions"
    target="_blank" rel="noopener noreferrer">Discussions</a>, so you keep your
    words and I keep no database.</p>
    <div class="giscus" data-giscus${attrs}>
      <noscript>Comments need JavaScript. You can also
      <a href="https://github.com/${escapeHtml(c.repo)}/discussions">read and post them on GitHub</a>.</noscript>
    </div>
  </section>`
}
