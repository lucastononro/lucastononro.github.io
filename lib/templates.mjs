...
function footer() {
  ...
  <div class="footer-grid">
    <div>
      ...
      built by a few hundred lines of Node, deployed by pushing to <code>main</code>
      and hoping.</p>
    </div>
...
export function homePage(posts, tagMap, chips) {
  const body = `
${hero(posts)}
${posts.length
    ? feedSection(posts, tagMap, chips)
    : `<section class="empty">
  <p class="section-label"><span>The feed</span></p>
  <article class="empty-card">
    <h2 class="empty-title">Nothing published yet.</h2>
    <p class="empty-body">The lights are on and nobody's home. The writing is
    the part that takes longer than the build.</p>
...
/**
 * The feed. Filter chips are real links to tag pages, so the thing works with
 * JavaScript switched off; theme.js upgrades them to filter in place.
 */
function feedSection(posts, tagMap, chips) {
  const featured = posts[0]
  const rest = posts.slice(1)

  return `<section class="featured" id="feed">
  <p class="section-label"><span>The feed</span></p>
  <p class="section-blurb">Everything, newest first. The ones tagged
  <em>skill</em> have an installable repository behind them; the rest is thinking
  out loud with the receipts attached.</p>
  ${filterBar(tagMap, chips, posts.length)}
  <div data-filterable>
    <ol class="entries entries-lead">
${[featured, ...rest].map((post, i) => entryRow(post, i)).join('\n')}
    </ol>
  </div>
</section>`
}

function filterBar(tagMap, chips, total) {
  const chipHtml = chips
    .map((tag) => `<a class="filter-chip" data-tag="${escapeHtml(tag)}"
      href="${u(`/tags/${tagSlug(tag)}/`)}">${escapeHtml(tag)}<span>${tagMap.get(tag).length}</span></a>`)
    .join('')
...
function hero(posts) {
  ...
  <div class="hero-text">
    <p class="eyebrow">Personal blog · ${stat} · no newsletter</p>
    ...
    <p class="hero-body">
      ...
      <em>nobody can see</em>. Both get written up here, with equal enthusiasm
      and roughly equal length.
    </p>
    <p class="hero-byline"><strong>${escapeHtml(site.author.name)}</strong> — ${escapeHtml(site.author.bio)}</p>
  </div>
  <figure class="portrait">
    <img class="portrait-img" src="${u('/portrait.png')}" width="460" height="460"
         alt="Lucas Tonon holding a small brown dog, both looking at the camera">
    <figcaption>
      <b>Lucas Tonon</b>
      <span>code review handled by the dog</span>
    </figcaption>
  </figure>
</section>`
}
...
<div class="page-head">
  ...
  ${filters ? filterBar(filters.tagMap, filters.chips, posts.length) : ''}
</div>
