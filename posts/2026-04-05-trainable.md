...
[Trainable](https://github.com/lucastononro/trainable) is a studio where agents do the data science. Start with the demo — it's faster than anything I can write here.
...
Every company I've worked with already has the data. That was the last decade's project and it worked: the warehouse exists, the events are instrumented, the nightly job runs, and storage costs less per terabyte than the meeting about it.

What almost none of them have is the person who turns that table into a model somebody will actually deploy. That person knows which distribution to be suspicious of, reads a 0.99 AUC as a leaked column rather than a result, and insists on beating the dull benchmark before anyone is allowed to try something interesting. They're scarce, expensive, and slow to hire — check how long your last data-scientist req stayed open. So the table sits there. The bottleneck was never storage. It hasn't been compute for years. It's the calendar of about four people.

Most of what fills that calendar isn't the judgment — it's the two weeks afterwards. Profiling every column, arguing with the missingness pattern, choosing encodings, splitting the data once and meaning it, fitting the boring baseline, thirty Optuna trials on top, writing up the leaderboard and the feature importances. Those two weeks are what Trainable automates, and I want to be exact about where it stops. It doesn't frame the problem, decide whether the model should exist, or notice that the label you were handed is a proxy for something you'd rather not predict. A human still picks the target, says what a false negative costs, and reads the review. The roster includes a `reviewer` agent whose entire brief is to disagree with the others — leakage, temporal leakage, features derived from the target. It's there because judgment is the part that needs checking, not the part that got replaced.

The input doesn't have to be a table, either. `sample-data/` ships seven datasets: six are a single CSV with a guessable header — Titanic, telco churn, wine quality, California housing, heart failure, employee attrition — and the seventh is 1,765 photographs of cars with a COCO annotation file beside them. Nothing in the upload path inspects a file: bytes go to S3 and to the Modal Volume under whatever relative path the browser sent, the content type falls back to `application/octet-stream`, and the agent opens them with Python. The honest limit is the paved road rather than the door: the prep explorer and the tabular training skill both want parquet splits and a named target, so a detector gets the sandbox, a GPU and a live grid of prediction overlays, but no pre-written methodology.
...
`pip install trainable-ai && trainable init`, and a wizard handles Docker and keys. What took the time isn't the agent roster. It's what happens after an agent decides to run some Python. The `execute-code` skill hands a string to `services/sandbox.py:run_code`, which builds a fresh Modal Sandbox per call:
...
Every argument there is a decision. `-u` because buffered stdout defeats the streaming below. The image is `debian_slim(python_version="3.11")` with pandas, scikit-learn, xgboost, lightgbm, duckdb, optuna, shap, statsmodels, CPU-wheel torch and `tensorflow-cpu` preinstalled — memoized in a module global, so the definition is assembled once per backend process and Modal serves the built layers to every sandbox after that. `gpu` and `timeout` come from a profile on the project row, so moving a project onto an A10G is a settings modal and the next call picks it up.[^gpu] When Modal kills a sandbox at the 600-second mark the handler doesn't fail the run: it returns a tool result with `is_error=True` and prose explaining the options — split the work, shrink the data, or retry with `heavy=true`.
...
S3 (MinIO in dev) is the second tier and pointedly not the authority. The volume is primary because it's what sandboxes mount. S3 takes a mirror of the finished workspace, and gets read back only to surface files that were uploaded but never made it into a sandbox, reported per file as `in_sandbox: false`.
...
`run_code` consumes the sandbox's stdout chunk by chunk, buffers it into lines, and sends each through `parse_stdout_line`. Recognized envelopes — scalar metrics, a `chart_config`, or a rich event such as an image, table or confusion matrix — get persisted and published; everything else is just output the model will read back. The whitelist of rich types is deliberately small. A new one needs a frontend renderer first; otherwise a typo in agent code quietly fills the table with rows nothing can draw.

Publishing means a per-session `asyncio.Queue` in an in-process broadcaster, drained by exactly one SSE route. The routers never learn the event types; they call `broadcaster.publish`, and the generator behind the endpoint is thirteen lines long (I counted).

This is the part that changes how the thing feels, and the part I'd defend to somebody who cares about none of the above. A chart that appears while the fit is still running is a chart you check at minute two, when the wrong column is still cheap to fix!! A report twenty minutes later is something you audit; a metric arriving now is something you supervise.
...
[^gpu]: Compute choices are coerced to a known set — `cpu`, `T4`, `L4`, `A10G`, `A100-40GB`, `A100-80GB`, `H100` — and anything unrecognized falls back to `cpu`, so that a misspelled value never silently lights up an expensive A100.
...
I keep it up because the framing in that title is the one I still hold; everything that changed since is underneath it. A different path to the same deployed model: the labor is automatable, the judgment is not, and the interesting engineering is the part nobody demos — a machine per call, a directory that outlives it, and a stream that lets you catch a mistake at minute two instead of in the write-up.

## State as code, not summaries

The question underneath all of this: how do you let a language model run arbitrary code for twenty minutes without the run becoming unauditable? My answer turned out to be a shape of storage, not a permission model: destroy the machine after every call, keep the directory. Everything a run produced is a file on the volume with a path, so a recursive diff and a classification by extension are enough to build the index the UI reads. Nothing had to be declared up front. No agent had to remember to register its own output.

The transferable idea is the smaller one. When an agent promotes a helper out of a notebook into `src/loaders.py` and a later agent imports it, the second one isn't working from a paraphrase of the first one's reasoning — it's running the same code, and it fails loudly if that code was wrong. Summaries drop exactly the details that break. Delete the volume and what remains is a transcript of a data science project: fluent, ordered, and reproducible by nobody.

---

The agents are the demo; the two weeks are the point.
